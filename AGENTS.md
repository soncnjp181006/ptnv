# Quy Chuẩn Hệ Thống & Quản Trị Dự Án (.agent-system/memory/)

Tài liệu này là **Chỉ Thị Tối Cao (System Governance Directive)** dành cho tất cả các AI Coding Agents (OpenCode, Kilo Code, Claude Code, Cursor, Antigravity) khi hoạt động trong workspace này.

---

## ⚠️ BẮT BUỘC NẠP BỘ NHỚ QUẢN TRỊ (MANDATORY MEMORY INGESTION)

Mỗi khi nhận lệnh từ người dùng, thực thi slash command hoặc thực hiện bất kỳ tác vụ lập trình nào, Agent **BẮT BUỘC PHẢI ĐỌC VÀ TUÂN THỦ** các tệp quy chuẩn tại thư mục `.agent-system/memory/`:
...
## 🚫 CÁC NGUYÊN TẮC BẤT BIẾN (SYSTEM INVARIANTS)

1. **Host-Centric AI**: Agent hiểu rằng Model và Runtime thuộc sở hữu của Host IDE. Không tìm kiếm API key riêng của framework.
